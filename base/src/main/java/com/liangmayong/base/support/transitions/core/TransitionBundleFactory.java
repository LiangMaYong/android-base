...
public class TransitionBundleFactory {
    ...
    private static String saveImage(final Context context, final Bitmap bitmap) {
        File file = ContextUtils.getApplication().getDir("activity_transition", Context.MODE_PRIVATE);
        if (!file.exists()) {
            file.mkdirs();
        }
        final String imageSavePath = file.getPath();
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                Boolean isDebug = (Boolean) TransitionUtils.getBuildConfigValue(context, "DEBUG");
