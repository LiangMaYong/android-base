...
public class LayoutCreator extends WriteCommandAction.Simple {
    ...
    public LayoutCreator(PsiFile file, PsiClass clazz, String command, ArrayList<Element> elements, String layoutFileName, String fieldNamePrefix, boolean bind, boolean createHolder) {
    ...
    protected void generatorBindView() {
        for (Iterator<Element> iterator = mElements.iterator(); iterator.hasNext(); ) {
            ...
            mClass.add(mFactory.createFieldFromText(Config.BindView + "(" + element.getFullID() + ")\nprivate " + element.name + " " + element.getFieldName() + ";", mClass));
        }
    }

    private void generatorBindOnClick() {
        ...
        if (clickableElements.size() > 0) {
            ...
            buider.append(Config.BindOnClick + "({");
    ...
    private void generatorBindLongOnClick() {
        List<Element> clickableElements = new ArrayList<Element>();
        for (Element element : mElements) {
        ...
        if (clickableElements.size() > 0) {
            StringBuilder buider = new StringBuilder();
            StringBuilder caseBuider = new StringBuilder();
            buider.append(Config.BindOnLongClick + "({");
            for (int i = 0; i < clickableElements.size(); i++) {
                buider.append(clickableElements.get(i).getFullID());
                if (i < clickableElements.size() - 1) {
                    buider.append(",");
    ...
    private void generatorLayoutCode(String contextName, String findPre) {
        List<Element> editTextElements = new ArrayList<Element>();
        List<Element> clickableElements = new ArrayList<Element>();
        List<Element> itemClickableElements = new ArrayList<Element>();
