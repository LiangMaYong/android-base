package com.liangmayong.base.support.deamon;
...
public class NativeRuntime {
     ...
     */
    public String RunExecutable(Application application, String pacaageName, String filename,
                                String alias, String args) {
        String dataDir = application.getApplicationInfo().dataDir;
        String path = dataDir + pacaageName;
        ...
        if (!new File(dataDir + alias).exists()) {
            RunLocalUserCommand(application, pacaageName, cmd4, sb_result); // 拷贝lib/libtest.so到上一层目录,同时命名为test.
            sb_result.append(";");
        }
        RunLocalUserCommand(application, pacaageName, cmd3, sb_result); // 改变test的属性,让其变为可执行
        sb_result.append(";");
        RunLocalUserCommand(application, pacaageName, cmd2_a1, sb_result); // 执行test程序.
        sb_result.append(";");
     ...
     */
    public boolean RunLocalUserCommand(Application application,
                                       String pacaageName, String command, StringBuffer sb_out_Result) {
        ...
        try {
            ...
            outputStream.writeBytes("cd "
                    + application.getApplicationInfo().dataDir + pacaageName
